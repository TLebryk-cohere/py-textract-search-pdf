...
def manual_get_job_res(bucket, prefix):
    """A way to read text even without job ID;
        helpful if you've moved the result location or don't konw if
        it's an analyze or detect_text job.

        :param prefix: should be directory where pages are stored.
    """
    # get contents of bucket
    # result = self.aws.s3_client.get_object(Bucket=bucket, Key=FILE_TO_READ) 
    
    my_bucket = boto3.resource('s3').Bucket(bucket)
    pg_nums = []
    for object_summary in my_bucket.objects.filter(Prefix=prefix):
        suffix = object_summary.key.split("/")[-1]
        if suffix.isdigit():
            pg_nums.append(int(suffix))
    npg = max(pg_nums)
    # print(npg)
    pgs = []
    for i in range(1, npg+1):
        result = boto3.client('s3').get_object(Bucket=bucket, Key=os.path.join(prefix, str(i))) 
        pg = json.loads(result["Body"].read().decode())
        pgs.append(pg)
    return pgs
